package com.abidimi.bookstore.runner;
...
import com.abidimi.bookstore.domain.book.review.permission.ReviewPermissionService;
...
public class DatabaseInitializer implements CommandLineRunner {
    ...
    private final ReviewPermissionService reviewPermissionService;
    ...
    public void run(String... args) {
        ...
        reviewService.saveReviews(getReviews());
//        getReviews().forEach(reviewService::saveReview);

        reviewPermissionService.saveReviewPermissions(getReviewPermissions());
//        getReviewPermissions().forEach(reviewPermissionService::saveReviewPermission);
    ...
    private List<Review> getReviews() {
        Book book1 = bookService.validateAndGetBook("9781603090773");
        Book book2 = bookService.validateAndGetBook("9781603090698");
        Book book3 = bookService.validateAndGetBook("9781891830372");

        User adminUser = userService.validateAndGetUserByUsername("admin");
        User userUser = userService.validateAndGetUserByUsername("user");

        return List.of(
            new Review("Admin Review 1", false, book1, adminUser),
            new Review("Admin Review 2", true, book2, adminUser),
            new Review("Admin Review 3", false, book3, adminUser),
            new Review("User Review 1", false, book1, userUser),
            new Review("User Review 2", true, book2, userUser),
            new Review("User Review 3", false, book3, userUser)
        );
    }

    private List<ReviewPermission> getReviewPermissions() {
        Review review2 = reviewService.validateAndGetReview(2L);
        Review review3 = reviewService.validateAndGetReview(3L);

        User adminUser = userService.validateAndGetUserByUsername("admin");
        User userUser = userService.validateAndGetUserByUsername("user");

        return List.of(
            new ReviewPermission(review2, userUser, true, false, false),
            new ReviewPermission(review3, userUser, true, false, false)
        );
    }
