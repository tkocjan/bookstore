...
import com.abidimi.orderapi.security.SecurityConfig;
import com.abidimi.orderapi.security.TokenProvider;
import com.abidimi.orderapi.domain.user.User;
import com.abidimi.orderapi.domain.user.UserNotFoundException;
...
class UserControllerTest {
  ...
  void getMe_returns200AsUser() throws Exception {
    ...
    mockMvc
        .perform(get("/api/users/me").with(user(userDetails)))
  ...
  void getMe_returns200AsAdmin() throws Exception {
    ...
    mockMvc
        .perform(get("/api/users/me").with(user(adminDetails)))
        .andExpect(status().isOk())
  ...
  void getUser_returns404WhenNotFound() throws Exception {
    ...
    mockMvc.perform(get("/api/users/ghost")).andExpect(status().isNotFound());
  }

  @Test
  @WithMockUser(username = "user", authorities = "USER")
  void getUser_returns403AsUser() throws Exception {
    mockMvc.perform(get("/api/users/alice")).andExpect(status().isForbidden());
  }
  ...
  void deleteUser_returns403AsUser() throws Exception {
    mockMvc.perform(delete("/api/users/alice")).andExpect(status().isForbidden());
  }

  @Test
  void deleteUser_returns401WhenUnauthenticated() throws Exception {
    mockMvc.perform(delete("/api/users/alice")).andExpect(status().isUnauthorized());
  }
}
