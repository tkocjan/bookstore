...
import com.abidimi.orderapi.rest.dto.LoginRequest;
import com.abidimi.orderapi.rest.dto.SignUpRequest;
...
class AuthControllerTest {
  ...
  void signUp_returns201WithTokenOnSuccess() throws Exception {
    ...
    when(userService.saveUser(any(User.class)))
        .thenReturn(new User("newuser", "encoded", "New User", "new@example.com", Role.USER));
  ...
  void signUp_returns409WhenUsernameAlreadyUsed() throws Exception {
    SignUpRequest request =
        new SignUpRequest("existinguser", "password", "Existing", "other@example.com");
  ...
  void signUp_returns409WhenEmailAlreadyUsed() throws Exception {
    SignUpRequest request =
        new SignUpRequest("newuser", "password", "New User", "existing@example.com");
    when(userService.hasUserWithUsername("newuser")).thenReturn(false);
