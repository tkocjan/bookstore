package com.abidimi.orderapi.security;
...
import com.abidimi.orderapi.rest.PublicController;
...
class CorsConfigTest {
  ...
  void simpleGet_allowedOrigin_receivesAllowOriginHeader() throws Exception {
    mockMvc()
        .perform(get("/public/numberOfUsers").header("Origin", "http://localhost:3000"))
        .andExpect(header().string("Access-Control-Allow-Origin", "http://localhost:3000"))
        .andExpect(header().string("Access-Control-Allow-Credentials", "true"));
  }
  ...
  void simpleGet_disallowedOrigin_doesNotReceiveAllowOriginHeader() throws Exception {
    mockMvc()
        .perform(get("/public/numberOfUsers").header("Origin", "http://evil.example.com"))
        .andExpect(header().doesNotExist("Access-Control-Allow-Origin"));
  }
