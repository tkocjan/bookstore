...
import com.abidimi.bookstore.domain.book.BookNotFoundException;
import com.abidimi.bookstore.domain.book.BookService;
import com.abidimi.bookstore.rest.dto.CreateBookRequest;
import com.abidimi.bookstore.security.CustomUserDetailsService;
...
class BookControllerTest {
  ...
  void getBooks_noText_asUser_returns200() throws Exception {
    when(bookService.getBooks())
        .thenReturn(List.of(new Book("111", "Alpha"), new Book("222", "Beta")));
  ...
  void getBooks_withText_asUser_returns200() throws Exception {
    when(bookService.getBooksContainingText("spy"))
        .thenReturn(List.of(new Book("999", "Super Spy")));

    mockMvc
        .perform(get("/api/books").param("text", "spy"))
        .andExpect(status().isOk())
  ...
  void deleteBook_asAdmin_returns204() throws Exception {
    ...
    mockMvc.perform(delete("/api/books/123")).andExpect(status().isNoContent());
  }

  @WithMockUser(authorities = "ADMIN")
  @Test
  void deleteBook_notFound_returns404() throws Exception {
    when(bookService.validateAndGetBook("000"))
        .thenThrow(new BookNotFoundException("Book with isbn 000 not found"));

    mockMvc.perform(delete("/api/books/000")).andExpect(status().isNotFound());
  }

  @WithMockUser(authorities = "USER")
  @Test
  void deleteBook_asUser_returns403() throws Exception {
    mockMvc.perform(delete("/api/books/123")).andExpect(status().isForbidden());
  }

  @Test
  void deleteBook_unauthenticated_returns401() throws Exception {
    mockMvc.perform(delete("/api/books/123")).andExpect(status().isUnauthorized());
  }
}
